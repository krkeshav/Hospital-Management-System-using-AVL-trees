...
void export_recursive(FILE *ptr,patient *node) 
{ 
    if(node != NULL) 
    { 
        export_recursive(ptr,node->left);
        fprintf(ptr,"%d %s %s %s %d %c %s %s %s\n",node->patientID,node->admit_date,node->name,node->dob,node->age,node->gender,node->phone,node->disease,node->treatment);   
        export_recursive(ptr,node->right); 
    }
}

void export(patient* root)
{
    FILE *ptr;
    ptr=fopen("Sorted_Records.txt","w");
    export_recursive(ptr, root);
    fclose(ptr);
}
